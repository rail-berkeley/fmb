            ...
            if True:
                episode.append({
                    'observation': {
                        ...
                        'object_id': np.int_(data['object_id'][i]).astype(np.uint8),
                    },
                    ...
                    'language_instruction': _parse_primitive_instruct(data['primitive'][i], data['object_id'][i]),
                    'language_embedding': INSTRUCT_EMBEDS[_parse_primitive_instruct(data['primitive'][i], data['object_id'][i])],
                })
            #except:
            #    return None
...
class FmbMultiObjectDataset3(tfds.core.GeneratorBasedBuilder):
    ...
    def _split_paths(self):
        """Define data splits."""
        all = glob.glob('/media/nvmep3p/fmb2/np_release/multi_object_manipulation/board_3/*.npy')
        return {
                'train': all,
        }
