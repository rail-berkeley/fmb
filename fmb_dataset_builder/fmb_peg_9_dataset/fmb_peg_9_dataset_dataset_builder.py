            ...
            if True:
                episode.append({
                    'observation': {
                        ...
                        'image_side_1_depth': data['obs/side_1_depth'][i].astype(np.uint16),
                        'image_side_2_depth': data['obs/side_2_depth'][i].astype(np.uint16),
                        'image_wrist_1_depth': data['obs/wrist_1_depth'][i].astype(np.uint16),
                        'image_wrist_2_depth': data['obs/wrist_2_depth'][i].astype(np.uint16),
                        'state': data['obs/q'][i].astype(np.float32),
                        'state_vel': data['obs/dq'][i].astype(np.float32),
                        ...
                        'peg_id': np.int_(data['object_info']['shape']).astype(np.uint8),
                        'color_id': np.int_(data['object_info']['color']).astype(np.uint8),
                        'length_id': np.int_(LENGTH_IDS[data['object_info']['length']]).astype(np.uint8),
                        'size_id': np.int_(SIZE_IDS[data['object_info']['size']]).astype(np.uint8),
                    },
...
class FmbPeg1Dataset(tfds.core.GeneratorBasedBuilder):
    """DatasetBuilder for example dataset."""

    VERSION = tfds.core.Version('1.0.0')
    RELEASE_NOTES = {
      '1.0.0': 'Initial release.',
      '2.0.0': 'Removed depth images.',
    }
            ...
            features=tfds.features.FeaturesDict({
                'steps': tfds.features.Dataset({
                    'observation': tfds.features.FeaturesDict({
                        ...
                        'length_id': tfds.features.Scalar(
                            dtype=np.uint8,
                            doc='Length ID of the object being manipulated.'
                        ),
                        'size_id': tfds.features.Scalar(
                            dtype=np.uint8,
    ...
    def _split_paths(self):
        """Define data splits."""
        return {
                'train': glob.glob('/media/nvmep3p/fmb2/processed_demos/peg/np/1*.npy'),
        }
