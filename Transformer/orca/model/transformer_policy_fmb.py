...
class FMBTransformerPolicy(nn.Module):
    """
    ...
    def transformer_call(
        ...
        padding_mask = self.get_token_padding_mask(input_tokens)
        output = self.transformer(
            input_tokens, attention_mask=attention_mask, padding_mask=padding_mask, train=train
        )
    ...
    def generate_masks(self):
        ...
        causal_mask = jnp.ones((self.total_tokens, self.total_tokens))
    ...
    def get_tokens(self, observations, train: bool = False):
        """
        ...
        obs_tokens = [
            tok(observations, train=train) for tok in self.observation_tokenizers
        ]
        ...
        if len(self.task_tokenizers) > 0:
            ...
            task_tokens = [
                tok(observations, train=train) for tok in self.task_tokenizers
            ]
        ...
        action_tokens = jnp.zeros(
            (
                *observations["image_wrist_1"].shape[:1],
    ...
    def assemble_input_tokens(self, task_tokens, obs_tokens, action_tokens):
        ...
        """
        tokens = jnp.concatenate([task_tokens, obs_tokens, action_tokens], axis=1)
        return tokens

    def get_token_padding_mask(self, tokens):
        padding_mask = jnp.all(tokens == 0, axis=-1)
        return padding_mask
